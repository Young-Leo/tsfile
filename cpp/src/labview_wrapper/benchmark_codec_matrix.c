...
/* The C wrapper currently exports this initializer from libtsfile.dll but does
 * not declare it in tsfile_cwrapper.h.  Initialize once before overriding the
 * global codecs; otherwise the first writer open restores the defaults. */
extern void init_tsfile_config(void);
...
static int configure_codecs(uint8_t value_encoding,
                            uint8_t value_compression,
                            uint8_t time_encoding,
                            uint8_t time_compression) {
    return check_status(set_datatype_encoding(TS_DATATYPE_DOUBLE,
                                              value_encoding),
...
static LV_Handle wrapper_open(const char* output_path, char** column_names,
    ...
    for (col = 0; col < ncols; ++col) {
        if (!check_status(lv_tsfile_schema_builder_add_column(
                              builder, column_names[col], LV_TYPE_DOUBLE,
                              LV_CAT_FIELD),
...
int main(int argc, char** argv) {
    ...
    for (block = 0; block < block_count; ++block) {
        ...
        timestamps =
            (int64_t*)malloc((size_t)rows_per_block * sizeof(int64_t));
        ...
        if (mode == MODE_WRAPPER) {
            ...
            if (!check_status(lv_tsfile_write_block_f64(
                                  wrapper_writer, timestamps, values,
                                  rows_per_block, ncols),
